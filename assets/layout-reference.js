(() => {
  ...
  const dialog=document.createElement('dialog');dialog.className='layout-reference-dialog';
  ...
  notice.innerHTML='<p role="status" aria-live="polite" aria-atomic="true"></p><button type="button" data-refresh hidden>刷新页面</button><button type="button" data-dismiss hidden aria-label="关闭进度提示">×</button>';
  ...
  refreshPage.onclick=()=>location.reload();dismissNotice.onclick=()=>{notice.hidden=true;};
  const syncConfirm=()=>{q('[data-confirm]').disabled=busy||importing||!data||!!activeJob;};
  ...
  async function trackJob(initial){
    ...
    activeJob=null;rememberJob(null);syncConfirm();
  }
  ...
  async function importUrl(){
    ...
    }catch(error){if(version===importVersion)q('[data-status]').textContent=(timedOut?'读取超时，请重试或直接粘贴截图':controller.signal.aborted?'已取消读取':error.message)+(previous?'。仍保留上一张参考图，可确认使用或粘贴替换。':'');}
  ...
  q('[data-confirm]').onclick=async()=>{if(busy||importing||!data||activeJob)return;busy=true;open.disabled=true;syncConfirm();q('[data-file]').disabled=true;const target=page;
    designSwitch.disabled=true;
    q('[data-status]').textContent='正在提交修改…';
    try{const job=await request('/jobs',{method:'POST',body:JSON.stringify({page:target.page,image:data,sourceUrl:loadedUrl,instruction:q('[data-instruction]').value,respectDesign:designSwitch.checked})});
    ...
    finally{busy=false;open.disabled=false;designSwitch.disabled=false;syncConfirm();q('[data-file]').disabled=false;}
  };
