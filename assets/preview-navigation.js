...
(() => {
  ...
  function revealCurrent() {
    ...
    if (!row) return;
    for (const { details, rows } of groups) {
      const active = rows.includes(row);
      details.classList.toggle('current-chapter', active);
      if (active && row.dataset.page !== preservedPage) details.open = true;
    }
    ...
    if (small) small.textContent = `FY27 / ${chapter} · 第 ${pageLabel(slides[position].page)} 页，共 ${slides.length} 页`;
    if (position === 0) { nav.scrollTop = 0; return; }
    const bounds = nav.getBoundingClientRect();
    const details = row.closest('details');
    const rect = (!details || details.open ? row : details.querySelector('summary')).getBoundingClientRect();
  ...
  // A closed chapter is one navigation stop; open chapters expose every page.
  window.addEventListener('keydown', event => {
    ...
    if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
      ...
      current.details.open = event.key === 'ArrowRight';
      if (!current.details.open && current.details.contains(document.activeElement)) current.details.querySelector('summary').focus({ preventScroll: true });
      revealCurrent();
      return;
    }
    const visible = groups.flatMap(item => item.details.open ? item.rows : item.rows.slice(0, 1));
    const position = visible.indexOf(current.details.open ? row : current.rows[0]);
    const next = visible[position + (event.key === 'ArrowUp' ? -1 : 1)];
    if (!next) return;
    preservedPage = next.dataset.page;
    host.goTo(slides.findIndex(slide => String(slide.page) === next.dataset.page));
    revealCurrent();
    if (nav.contains(document.activeElement)) {
      const details = next.closest('details');
      (details.open ? next : details.querySelector('summary')).focus({ preventScroll: true });
    }
  }, true);
  const hint = document.querySelector('.bottom .hint');
  if (hint) {
    hint.setAttribute('aria-label', '快捷键：上下翻页，左键收起章节，右键展开章节，F 全屏，G 总览');
    const item = hint.querySelector('.hint-item');
    if (item) item.textContent = '↑↓ 翻页　← 收起　→ 展开';
  }
