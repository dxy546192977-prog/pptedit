...
async function scenario(fail=false){
 ...
 else{assert.equal(slide.notes[0],'优化讲稿');assert.equal(storage.size,0);assert.equal(rendered,1);}
}
await scenario();await scenario(true);console.log('Notes editor: closes immediately, prevents duplicate submissions, updates notes, preserves failed drafts.');
