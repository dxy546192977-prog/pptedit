...
async function scenario(fail=false,mode='instruction'){
 ...
 if(fail){assert.equal(slide.notes[0],'原稿');assert.equal(storage.size,1);assert.match(progress.textContent,/草稿已保留/);button.onclick();assert.equal(elements.get('[data-draft]').value,'新增讲稿');}
 else{assert.equal(slide.notes[0],'优化讲稿');assert.equal(storage.has('ppt-notes-'+mode+':/deck:6'),false);assert.equal(rendered,1);}
}
await scenario();await scenario(true);await scenario(false,'draft');console.log('Notes editor: both modes, duplicate submission prevention, notes update and failed draft recovery passed.');
